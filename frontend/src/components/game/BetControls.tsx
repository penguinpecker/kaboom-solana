...
export function BetControls() {
  ...
  const authenticated = connected && !!publicKey;

  useEffect(() => {
    if (!publicKey || !connection) return;
    let cancelled = false;
    const f = async () => {
      try { const b = await connection.getBalance(publicKey); if (!cancelled) setWalletBalance(b / LAMPORTS_PER_SOL); } catch {}
    };
  ...
  const handleStart = () => {
    if (!authenticated) { setVisible(true); return; }
