...
export function GameProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    callApi("/api/health").then(() => setUseOnChain(true)).catch(() => {
      console.warn("API unavailable, using local demo mode");
      setUseOnChain(false);
    });
  }, []);
  ...
  const startGame = useCallback(async () => {
    ...
    try {
      if (useOnChain) {
        ...
        const commitData = await callApi("/api/commit", {
        ...
        setState(prev => ({
          ...
          revealedTiles: new Set(), safeTiles: new Set(), mineTiles: new Set(),
          optimisticTiles: new Set(), multiplier: 1.0, payout: 0, pendingTile: null,
        }));
      } else {
        localMines = generateLocalMines(state.mineCount);
        setState(prev => ({
          ...prev, gameId: BigInt(Date.now()), status: "playing",
          commitment: "local-demo", error: null,
          revealedTiles: new Set(), safeTiles: new Set(), mineTiles: new Set(),
          optimisticTiles: new Set(), multiplier: 1.0, payout: 0, pendingTile: null,
        }));
      }
  ...
  const revealTile = useCallback(async (index: number) => {
    ...
    try {
      if (useOnChain) {
        const data = await callApi("/api/reveal", {
          player: wallet.publicKey?.toBase58(), tileIndex: index,
        });
        if (data.isMine) {
          setState(prev => {
            ...
            const result: GameResult = {
              gameId: prev.gameId?.toString() || "0", player: wallet.publicKey?.toBase58() || "",
            ...
            saveResult(result);
            return { ...prev, status: "lost" as GameStatus, revealedTiles: newRevealed, mineTiles: newMines,
              pendingTile: null, lastTxHash: data.signature, sessionGames: prev.sessionGames + 1,
          ...
          setState(prev => {
            ...
            return { ...prev, status: newSafe.size >= totalSafe ? "won" as GameStatus : "playing" as GameStatus,
              revealedTiles: newRevealed, safeTiles: newSafe, multiplier: newMult,
        ...
        if (isMine) {
          setState(prev => {
            ...
            saveResult({ gameId: prev.gameId?.toString() || "0", player: "demo", won: false, bet: prev.bet,
              payout: 0, multiplier: 0, mineCount: prev.mineCount, tilesCleared: prev.safeTiles.size,
              txHash: "", timestamp: Date.now() });
            return { ...prev, status: "lost" as GameStatus, revealedTiles: newRevealed, mineTiles: newMines,
              pendingTile: null, sessionGames: prev.sessionGames + 1, sessionPnl: prev.sessionPnl - prev.bet };
          });
          setGameHistory(loadHistory());
        } else {
          setState(prev => {
            ...
            const newMult = calcMultiplier(newSafe.size, prev.mineCount);
            const totalSafe = GAME_CONFIG.GRID_SIZE - prev.mineCount;
            return { ...prev, revealedTiles: newRevealed, safeTiles: newSafe, multiplier: newMult,
              pendingTile: null, status: newSafe.size >= totalSafe ? "won" as GameStatus : "playing" as GameStatus };
          });
        }
      }
    } catch (err: any) {
  ...
  const cashOut = useCallback(async () => {
    ...
    try {
      ...
      if (useOnChain) {
        const settleData = await callApi("/api/settle", { player: wallet.publicKey?.toBase58() });
        setState(prev => {
          saveResult({ gameId: prev.gameId?.toString() || "0", player: wallet.publicKey?.toBase58() || "",
            won: true, bet: prev.bet, payout, multiplier: prev.multiplier,
            mineCount: prev.mineCount, tilesCleared: prev.safeTiles.size,
            txHash: settleData.signature || "", timestamp: Date.now() });
          return { ...prev, status: "won" as GameStatus, payout, lastTxHash: settleData.signature,
            sessionGames: prev.sessionGames + 1, sessionPnl: prev.sessionPnl + (payout - prev.bet) };
        });
      } else {
        setState(prev => {
          saveResult({ gameId: prev.gameId?.toString() || "0", player: "demo", won: true, bet: prev.bet,
            payout, multiplier: prev.multiplier, mineCount: prev.mineCount, tilesCleared: prev.safeTiles.size,
            txHash: "", timestamp: Date.now() });
          return { ...prev, status: "won" as GameStatus, payout,
            sessionGames: prev.sessionGames + 1, sessionPnl: prev.sessionPnl + (payout - prev.bet) };
        });
      }
      setGameHistory(loadHistory());
