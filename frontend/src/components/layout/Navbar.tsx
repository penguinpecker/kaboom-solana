...
export function Navbar() {
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    const f = async () => {
      try { const b = await connection.getBalance(publicKey); if (!cancelled) setWalletBal((b / LAMPORTS_PER_SOL).toFixed(2)); } catch {}
    };
    ...
    <>
      <header className="fixed top-0 w-full z-50 flex justify-between items-center px-6 h-16 bg-surface-container-low/90 backdrop-blur-xl shadow-[0_0_20px_rgba(208,188,255,0.1)]">
        ...
        <div className="flex items-center gap-4">
          ...
          <button onClick={() => isConnected ? open("profile") : setVisible(true)}
