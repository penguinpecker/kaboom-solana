...
function ProfileModal() {
  ...
  useEffect(() => {
    ...
    import("@solana/web3.js").then(({PublicKey:PK}) => connection.getBalance(new PK(publicKey.toBase58()))).then(b => setBal((b / LAMPORTS_PER_SOL).toFixed(3))).catch(() => {});
...
function DepositModal() {
  const { close } = useModal();
  const { toast } = useToast();
  const { wallets: w } = useWallets();
  const publicKey = w[0]?.address ? { toBase58: () => w[0].address } : null;
